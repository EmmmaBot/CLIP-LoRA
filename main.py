...
def main():
    ...
    # Print dataset sizes
    print(f"Dataset sizes (samples): Train={len(dataset.train_x)}, Val={len(dataset.val)}, Test={len(dataset.test)}")
    ...
    # Add training log details
        print(f"Train Loader Ready: Batch Size={args.batch_size}, Steps per Epoch={len(train_loader)}")
